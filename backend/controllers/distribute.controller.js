...
const distributeAsset = async (req, res) => {
  try {
    ...
    asset.distributedTo = userId;
    user.handlingAssets.push(assetId);

    await asset.save();
    await user.save();

    res.status(200).json({ message: "Asset successfully distributed" });
  } catch (error) {
  ...
  const getAllUserWithAssets = async (req, res) => {
    try {
      ...
      if (!users || users.length === 0) {
        return res.status(404).json({ message: "No users with assigned assets found" });
      }
...
const getAssetUser = async (req, res) => {
    try {
      ...
      if (!asset.distributedTo) {
        return res.status(200).json({ message: "Asset is not distributed to any user" });
      }
  ...
  const getAllDistributedAssets = async (req, res) => {
    try {
      const assets = await Asset.find({ distributedTo: { $exists: true, $ne: null } })
        .populate({
          path: "distributedTo",
          select: "firstname lastname email role", // Fetch user details
        })
        .select("productname producttype serialnumber status condition distributedTo")
        .lean();
